...
ORIGENES=( /etc/init.d/ /etc/mysql/ /etc/nagios-nrpe/ /etc/network/interfaces /etc/resolv.conf /var/log/ /root/scripts/ )

# Destinos del backup (array) dentro de /backups/daily/
DESTINOS=( servicios mysql nrpe conf-ip resolver logs scripts )

# Copia de base de datos
USER_BBDD="root"
PASS_BBDD="invernalia"
ALL_DUMP_MYSQL="yes"
DUMP_MYSQL="no"
NAME_BBDD=()
